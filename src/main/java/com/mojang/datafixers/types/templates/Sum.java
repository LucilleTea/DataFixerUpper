...
public final class Sum implements TypeTemplate {
    ...
    private Sum(final TypeTemplate f, final TypeTemplate g) {
        this.f = f;
        this.g = g;
    }
    ...
    public <FT, FR> Either<TypeTemplate, Type.FieldNotFoundException> findFieldOrType(final int index, @Nullable final String name, final Type<FT> type, final Type<FR> resultType) {
        ...
        return either.map(
            f2 -> Either.left(Pool.SUM_POOL.create(new Sum.CreateInfo(f2, g))),
            r -> g.findFieldOrType(index, name, type, resultType).mapLeft(g2 -> Pool.SUM_POOL.create(new Sum.CreateInfo(f, g2)))
        );
    }
